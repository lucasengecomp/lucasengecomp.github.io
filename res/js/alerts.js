...
Alerts.asker = function(title, opts){
    ...
    Alerts.open(null, {
        ...
        ready: function(modal){
            ...
            if(opts.afterReady) opts.afterReady();
            else{
    ...
    return new Promise(function(resolve, reject){
        ...
        Alerts.open(null, {
            ...
            ready: function(modal){
                ...
                if(opts.afterReady) opts.afterReady(modal);
                else{
...
// Monta um modal genérico
Alerts.generic = (html, obj = {}) => {
    ...
    Alerts.open(null, {
        ...
        ready: function (modal) {
            ...
            modal.find('.modal').append(html);

            if (typeof obj.onReady == 'function') obj.onReady(modal);
...
Alerts.centralize = function(modal, center){
    ...
    modal.css('left', center.x - (modal.outerWidth() / 2));
    modal.css('top', center.y - (modal.outerHeight() / 2));
...
Alerts.show = (txt, delay) => {

    Alerts.notify(I18n.get(txt), delay);
